...
public class ViewlikeController {
     ...
     */
    @GetMapping("/view/my")
    public CursorResult<ReviewPostRes> getRecentView(Long cursorId, Integer size) { // get all review(size 만큼의 리뷰글과 다음 리뷰글의 존재여부(boolean) 전달)
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        Member member = memberService.getMember(username);
        if(size == null) size = PAGE_SIZE;
        return viewlikeService.getRecentView(member, cursorId, PageRequest.of(0, size));
    }

    @GetMapping("/like/my")
    public CursorResult<ReviewPostRes> getLike(Long cursorId, Integer size) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        Member member = memberService.getMember(username);
        if(size == null) size = PAGE_SIZE;
        return viewlikeService.getLike(member, cursorId, PageRequest.of(0, size));
    }
}
